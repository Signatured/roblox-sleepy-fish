...
local TraitTypes = require(game.ReplicatedStorage.Game.Library.Types.Traits)
export type dir_schema = TraitTypes.dir_schema

local Children: typeof(workspace.__DIRECTORY.Traits) = require(game.ReplicatedStorage.DirectoryLoader):WaitForChild(script.Name)

local module: {[string]: TraitTypes.dir_schema} = {}
...
if game:GetService("RunService"):IsServer() and game:GetService("RunService"):IsStudio() then
    for _, dir in pairs(module) do
        local success, reason = pcall(function()
            ...
            assert(type(dir.TraitEarningsMultiplier) == "number")
            assert(type(dir.ApplyToModel) == "function")
        end)
        if not success then
