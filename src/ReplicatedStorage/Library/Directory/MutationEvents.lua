...
local MutationEventTypes = require(game.ReplicatedStorage.Game.Library.Types.MutationEvents)
export type dir_schema = MutationEventTypes.dir_schema

local Children: typeof(workspace.__DIRECTORY.MutationEvents) = require(game.ReplicatedStorage.DirectoryLoader):WaitForChild(script.Name)

local module: {[string]: MutationEventTypes.dir_schema} = {}
...
if game:GetService("RunService"):IsServer() and game:GetService("RunService"):IsStudio() then
    for _, dir in pairs(module) do
        local success, reason = pcall(function()
            ...
            assert(type(dir.Interval) == "number")
            assert(type(dir.Duration) == "number")
        end)
        if not success then
