...
local function refresh()
	...
	if timerLabel and timerLabel:IsA("TextLabel") then
		...
		task.spawn(function()
			while myToken == timerToken and TabController.GetCurrentTab and TabController.GetCurrentTab() == "DailyQuests" and timerLabel and timerLabel.Parent do
				local utc = workspace:GetServerTimeNow()
				local est = utc - (5 * 3600)
				local remain = 86400 - (est % 86400)
				if remain < 0 then remain = 0 end
				timerLabel.Text = string.format("New Daily Quests In: %s", Functions.FormatTime(remain))
				task.wait(1)
			end
		end)
	end

	if not DailyQuestsCmds then return end
	local data = DailyQuestsCmds.Get()
