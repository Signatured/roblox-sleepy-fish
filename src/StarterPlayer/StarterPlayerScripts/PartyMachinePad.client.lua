...
TagHook(TAG, function(instance: Instance)
    ...
    local conn = pad:AddEnterListener(function(_player)
        ...
        if not isStudio and not save.TutorialClaim then
        ...
        if TabController.GetCurrentTab() ~= "PartyMachine" then
            TabController.OpenTab("PartyMachine")
        end
    end)
    return function()
