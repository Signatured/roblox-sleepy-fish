...
local currentCategory: FishTypes.fish_type | "BloodMoon" | "Galaxy" | "Spooky" | "Haunted" | "YinYang" = "Normal"

local categories = {"Normal","Gold","Rainbow","Shiny","BloodMoon","Galaxy","Spooky","Haunted","YinYang"}
...
local function setCategoryButtons(root: Instance, onCategoryChanged: () -> ())
    ...
    local function wire(buttonName: string, cat: FishTypes.fish_type | "BloodMoon" | "Galaxy" | "Spooky" | "Haunted" | "YinYang")
...
local function realRender()
    ...
    for _, dir in ipairs(fishList) do
        ...
        if entry then
            ...
            elseif currentCategory == "YinYang" then hasSeen = entry.YinYang == true
            end
        end
        local countVal = 0
        if hasSeen then
